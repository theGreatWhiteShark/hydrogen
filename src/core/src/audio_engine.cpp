...
inline timeval AudioEngine::currentTime2()
{
	struct timeval now;
...
inline int AudioEngine::randomValue( int max )
{
	return rand() % max;
}

inline float AudioEngine::getGaussian( float z )
{
...
void AudioEngine::audioEngine_raiseError( unsigned nErrorCode )
{
 ...
 */
void AudioEngine::updateTickSize()
{
...
void AudioEngine::audioEngine_init()
{
...
void AudioEngine::audioEngine_destroy()
{
	// check current state
...
int AudioEngine::audioEngine_start( bool bLockEngine, unsigned nTotalFrames )
{
	if ( bLockEngine ) {
		lock( RIGHT_HERE );
	}
...
void AudioEngine::audioEngine_stop( bool bLockEngine )
{
	if ( bLockEngine ) {
		lock( RIGHT_HERE );
	}
...
inline void AudioEngine::audioEngine_process_checkBPMChanged(Song* pSong)
{
	if ( m_audioEngineState != STATE_READY
		 && m_audioEngineState != STATE_PLAYING ) {
		return;
...
inline void AudioEngine::audioEngine_process_playNotes( unsigned long nframes )
{
	Hydrogen* pHydrogen = Hydrogen::get_instance();
	Song* pSong = pHydrogen->getSong();
	...
	while ( !m_songNoteQueue.empty() ) {
		...
		if ( isNoteStart || isOldNote ) {
			...
			if ( noteInstrument->is_stop_notes() ){
				...
				getSampler()->note_on( pOffNote );
				delete pOffNote;
			}

			getSampler()->note_on( pNote );
...
void AudioEngine::audioEngine_seek( long long nFrames, bool bLoopMode )
{
	Hydrogen* pHydrogen = Hydrogen::get_instance();
	Song* pSong = pHydrogen->getSong();
...
inline void AudioEngine::audioEngine_process_transport()
{
	if ( m_audioEngineState != STATE_READY
	  && m_audioEngineState != STATE_PLAYING
	) return;
...
void AudioEngine::audioEngine_clearNoteQueue()
{
 ...
 * unlock and return.
 */
inline void AudioEngine::audioEngine_process_clearAudioBuffers( uint32_t nFrames )
{
...
int AudioEngine::audioEngine_process( uint32_t nframes, void* /*arg*/ )
{
	...
	if ( !tryLockFor( std::chrono::microseconds( (int)(1000.0*fSlackTime) ),
												   RIGHT_HERE ) ) {
	...
	float* out_L = getSampler()->__main_out_L;
	float* out_R = getSampler()->__main_out_R;
...
void AudioEngine::audioEngine_setupLadspaFX( unsigned nBufferSize )
{
 ...
 */
void AudioEngine::audioEngine_renameJackPorts(Song * pSong)
{
#ifdef H2CORE_HAVE_JACK
	// renames jack ports
	if ( ! pSong ) return;
...
void AudioEngine::audioEngine_setSong( Song* pNewSong )
{
...
void AudioEngine::audioEngine_removeSong()
{
	lock( RIGHT_HERE );
...
inline int AudioEngine::audioEngine_updateNoteQueue( unsigned nFrames )
{
	Hydrogen* pHydrogen = Hydrogen::get_instance();
	Song* pSong = pHydrogen->getSong();
			...
			if ( m_nSongPos == -1 ) {
				...
				if ( pSong->is_loop_enabled() == true ) {
					...
					if( m_pMidiDriverOut != nullptr ){
						m_pMidiDriverOut->handleQueueAllNoteOff();
					}
...
void AudioEngine::sequencer_setNextPattern( int pos )
{
	lock( RIGHT_HERE );
...
void AudioEngine::sequencer_setOnlyNextPattern( int pos )
{
	lock( RIGHT_HERE );
...
void AudioEngine::setCurrentPatternList( PatternList *pPatternList )
{
	lock( RIGHT_HERE );
...
void AudioEngine::togglePlaysSelected()
{
...
void AudioEngine::setSelectedInstrumentNumber( int nInstrument )
{
...
void AudioEngine::setSelectedPatternNumberWithoutGuiEvent( int nPat )
{
	Song* pSong = Hydrogen::get_instance()->getSong();
...
void AudioEngine::setSelectedPatternNumber( int nPat )
{
	if ( nPat == m_nSelectedPatternNumber ) {
		return;
	}
...
void AudioEngine::resetPatternStartTick()
{
...
void AudioEngine::audioEngine_noteOn( Note *note )
{
	// check current state
	if ( ( m_audioEngineState != STATE_READY )
		 && ( m_audioEngineState != STATE_PLAYING ) ) {
...
void AudioEngine::audioEngine_startAudioDrivers()
{
...
void AudioEngine::audioEngine_stopAudioDrivers()
{
...
void AudioEngine::audioEngine_restartAudioDrivers()
{
	audioEngine_stopAudioDrivers();
	audioEngine_startAudioDrivers();
}

int AudioEngine::aE_process( uint32_t nframes, void *arg ){
	return Hydrogen::get_instance()->getAudioEngine()->audioEngine_process( nframes, arg );
}
		...
		, m_pSynth( nullptr )
{
	...
	m_pMainBuffer_L = nullptr;
	m_pMainBuffer_R = nullptr;
	m_audioEngineState = Hydrogen::STATE_UNINITIALIZED;	
	m_nPatternStartTick = -1;
	m_nPatternTickPosition = 0;
	m_nSongSizeInTicks = 0;
	m_nRealtimeFrames = 0;
